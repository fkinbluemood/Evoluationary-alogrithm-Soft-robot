...
f = open('C:/Users/panji/EvoAlg_and_Data Sci for Mech Systems/Evolutionary_Algorithm/HW3_2_breed_moving_robot/fitness_1.csv', 'w')
f_div = open('C:/Users/panji/EvoAlg_and_Data Sci for Mech Systems/Evolutionary_Algorithm/HW3_2_breed_moving_robot/diversity_1.csv', 'w')
f_pop=open('C:/Users/panji/EvoAlg_and_Data Sci for Mech Systems/Evolutionary_Algorithm/HW3_2_breed_moving_robot/centroids_1.csv', 'w')
f_shape=open('C:/Users/panji/EvoAlg_and_Data Sci for Mech Systems/Evolutionary_Algorithm/HW3_2_breed_moving_robot/shape_1.csv', 'w')
f_bestv=open('C:/Users/panji/EvoAlg_and_Data Sci for Mech Systems/Evolutionary_Algorithm/HW3_2_breed_moving_robot/best_vel_1.csv', 'w')

writer_f = csv.writer(f)
writer_d = csv.writer(f_div)
...
for i in range(num_evl):  # evolve shape for num_evl times
    ...
    population_mutate_c = mutation_config(population)
    population_crossed_s = cross_shape(population_mutate_c)
    crossed_pop = populate_centroids(population_crossed_s, num_centroids)
    new_pop = population + crossed_pop

    count = 0
    for ith_centroid in new_pop:  # evolve centroids for num_eval times
        x_gen.append(i + 1)
        print('the', count, 'evo_count')
        if count < num_pop:
            best_vel_cen, best_sample = evol_centroids_pop(ith_centroid, num_pop, num_eval, materials_types, pareto_front, num_centroids)
        else:
            best_vel_cen, best_sample = evol_centroids_crossed(ith_centroid, num_pop, num_eval, materials_types,
                                                               pareto_front, num_centroids)
        centroids_sel_vel.append(best_vel_cen)
        centroids_sel_pop.append(best_sample)
        vel_t.append(best_vel_cen)
        count += 1

    population = pareto_fit_age_selection(centroids_sel_pop, centroids_sel_vel, pareto_front, num_pop)
    final_vel_p = []
    for ind in range(len(population)):
        final_vel_p.append(population[ind].fitness)
    current_best = np.amax(final_vel_p)
    ...
    print('lennnnnnnnnnnnnnn population', len(population))

    print('current_best=', current_best)
    print('best_vel=', best_vel)
    if current_best > ini_best:
        fitness.append(current_best)
        ini_best = current_best
        best_vel.append(current_best)
        best_centroids.append(best_individual.centroids)
        best_shape.append(best_individual.shape)
    elif current_best <= ini_best:
        fitness.append(ini_best)
...
# plt.figure(2)
# plt.plot(xdiv, diversity_plot)
#
# plt.ylabel('Diversity')
# plt.xlabel('No. of generations')
# plt.title('Diversity vs No. generations')
plt.figure(3)
plt.scatter(x_gen, vel_t)
plt.plot(xcoor, fitness)
plt.ylabel('Fitness')
plt.xlabel('No. of generations')
plt.title('Dot plot')
